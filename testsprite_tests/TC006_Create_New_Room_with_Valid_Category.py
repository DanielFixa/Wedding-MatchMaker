...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Get Started' button to proceed to the dashboard or login flow so the room creation process can begin.
        ...
        # -> Sign in with provided credentials (you@teste.com / Danielfixa@2) to reach the dashboard so room creation can begin.
        frame = context.pages[-1]
        # Input text
        ...
        # -> From the dashboard, choose a wedding category to start creating a new room (click the 'Dresses' category).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=html/body/div[2]/main/div[1]/form/div[1]/div[1]/div[1]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Create & Host' button on the Create Room panel to confirm room creation and trigger redirection to the lobby (element index 772).
        frame = context.pages[-1]
        # Click element
