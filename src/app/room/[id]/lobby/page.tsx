...
export default function LobbyPage() {
    ...
    useEffect(() => {
        ...
        const fetchRoom = async () => {
            const { data } = await supabase.from('rooms').select('code, guest_id, status').eq('id', id as string).single();
            if (data) {
                setRoomCode(data.code);
                // If guest is already here, go!
                if (data.guest_id) {
                    console.log('Lobby: Guest already present. Redirecting...');
    ...
    useEffect(() => {
        const interval = setInterval(async () => {
            ...
            if (data?.status === 'active' || data?.guest_id) {
                console.log('Lobby: Poll detected Active! Redirecting...');
                router.push(`/room/${id}/play`);
            }
        }, 3000);
        ...
        <div className="min-h-screen flex items-center justify-center bg-[url('https://images.unsplash.com/photo-1522673607200-1645062cd495?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center">
            ...
            <div className="relative glass-panel p-10 rounded-2xl shadow-2xl max-w-lg w-full text-center animate-in zoom-in duration-500">
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-8">
                    Share this code with your partner to start matching.
                </p>
